...
bool UcppFunctions::CopyPolygroupToMesh(UDynamicMeshComponent* InputComponent, UDynamicMeshComponent* TargetComponent, int32 Polygroup)
{
    if (InputComponent && InputComponent->GetMesh() && TargetComponent && TargetComponent->GetMesh())
    {
        const UE::Geometry::FDynamicMesh3* ReadMesh = InputComponent->GetMesh();
        ...
        FDynamicMesh3* Mesh = InputComponent->GetMesh();
        FDynamicMesh3* TMesh = TargetComponent->GetMesh();
        ...
        TargetComponent->NotifyMeshUpdated();
...
void UcppFunctions::NumPolygroups(UDynamicMeshComponent* InputComponent, TArray<int32>& PolygroupIDs)
{
    if (InputComponent)
    {
        UE::Geometry::FPolygroupLayer InputGroupLayer;
...
void UcppFunctions::MoveVertices(UDynamicMeshComponent* InputComponent, float Amount, int32 xAmount, int32 yAmount)
{
    if (InputComponent)
    {
        UE::Geometry::FPolygroupLayer InputGroupLayer;
...
void UcppFunctions::CalculateSurface(UDynamicMeshComponent* InputComponent)
{
